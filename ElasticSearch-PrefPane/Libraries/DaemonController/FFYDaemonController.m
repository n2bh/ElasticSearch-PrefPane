...
//
//     daemonController.startArguments = @"--basedir=/usr/local/Cellar/mysql/5.1.56";
//
...
// The pidPath is the PID file's absolute location.
@synthesize pidPath;
...
- (void)start {
  @try {
    ...
    if (startArguments) {
      NSString *trimmedArguments = [startArguments stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
      NSString *allArguments = [trimmedArguments stringByAppendingFormat:@" -p %@", pidPath];
      daemonTask.arguments = [allArguments componentsSeparatedByString:@" "];
    }
    else
      daemonTask.arguments = [NSArray arrayWithObjects:@"-p", [NSString stringWithFormat:@"%@", pidPath], nil];
...
- (void)stop {
  ...
  if (daemonTask) {
    ...
    // If not, get the PID from the file.
    if (![fileManager fileExistsAtPath:pidPath]) {
      ...
      NSString *pidString = [[NSString alloc] initWithData:[fileManager contentsAtPath:pidPath]
...
// Custom setter for launchPath to extract the binary name from the path.
...
// When setting the pidPath, means that the deamon to watch has changed. So this custom
// setters does what needed to invalidate timers, and prepare it to watch for another daemon.
- (void)setPidPath:(NSString *)thePidPath {
  if (pidPath != thePidPath) {
